...
ANALYSIS_PROMPT = (
    ...
    "근거에 없는 내용은 추측하지 마라. "
    "인용 규칙(반드시 지킬 것): 모든 핵심 주장·문장 끝에 그 근거가 된 자료 번호를 "
    "제공된 라벨 그대로 [논문 N]·[뉴스 N]·[웹 N] 형식으로 표기하라(예: ...수요가 급증한다[논문 2][뉴스 1]). "
    "근거를 특정할 수 없는 문장은 쓰지 마라. "
...
def _build_prompt(question, history=None, use_web=False):
    ...
    if is_analysis:
        tail = ("위 자료에 기반해 분석하라. 신뢰도는 논문 > 뉴스 > 웹 순으로 가중하되, "
                ...
                "반드시 각 주장 끝에 근거 번호를 [논문 N]/[뉴스 N]/[웹 N] 형식으로 인용 표기하라.")
    else:
